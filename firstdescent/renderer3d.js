...
if(renderer){
 ...
 else{const m=s.normalMaterial;s.object.material=m;const map=faces.alienMaterial?alienTextures[faces.alienMaterial]:faces.rock?rockTexture:organic?skinColor:faces.industrial?industrialColor:null,bump=faces.alienMaterial?alienTextures[faces.alienMaterial]:faces.rock?rockTexture:organic?skinTexture:faces.pilotHull?null:faces.capitalSurface?industrialColor:metalTexture;if(m.map!==map||m.bumpMap!==bump){const programChanged=!!m.map!==!!map||!!m.bumpMap!==!!bump;m.map=map;m.bumpMap=bump;if(programChanged)m.needsUpdate=true;}m.metalness=faces.rock||organic?0:faces.pilotHull?.32:faces.capitalSurface?.58:faces.industrial?.3:.64;m.roughness=faces.rock?.98:faces.pilotHull?.57:faces.alienMaterial==='chitin'?.53:organic?.76:faces.capitalSurface?.46:faces.industrial?.8:.34;m.bumpScale=faces.alienMaterial==='chitin'?.95:faces.alienMaterial==='flesh'?.23:faces.rock?1.7:organic?1.1:.3;}
