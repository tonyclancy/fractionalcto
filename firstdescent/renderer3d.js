...
if(renderer){
 ...
 function includeDrawBounds(data,faces,matrix){
  // CPU-articulated bosses may move beyond their bind sphere. Preserve their
  // full viewport; static/GPU-rigged meshes use the same padded bounds as culling.
  if(faces.dynamic){clipLeft=clipTop=0;clipRight=1440;clipBottom=760;return true;}
 ...
 function geometry(faces){if(geoCache.has(faces)){const cached=geoCache.get(faces);cached.lastUsed=renderFrame;if(faces.dynamic&&(!(faces.alienMaterial||faces.capitalHull)||cached.uploadFrame!==renderFrame)){cached.uploadFrame=renderFrame;const a=cached.geometry.attributes.position;const positions=a.array;for(let i=0;i<cached.sources.length;i++){const v=cached.sources[i],j=i*3;positions[j]=v[0];positions[j+1]=v[1];positions[j+2]=v[2];}a.needsUpdate=true;cached.normalFrame=(cached.normalFrame||0)+1;if(faces.painted===undefined&&cached.normalFrame%((window.flightEffectsQuality||1)<1?4:3)===0)smoothNormals(cached.geometry,cached.smoothGroups||[])}return cached}const sources=[],positions=[],colors=[],glow=[],flex=[],wet=[],textureWeight=[],blinkData=[],faunaJoints=[],uv=[],indices=[],lookup=new Map(),normalGroups=new Map();let organic=!!faces.skin;
 ...
 shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>', '#include <map_fragment>\n if(alienSurface>0.5){float relief=dot(diffuseColor.rgb,vec3(.2126,.7152,.0722));diffuseColor.rgb=mix(vec3(1.0),vec3(clamp(.32+relief*2.9,.28,1.35)),vTextureWeight);}');
 ...
 if(alienSurface>0.5)roughnessFactor=mix((alienSurface<1.5?.58:.49)+skinNoise(vSkinPosition*.24)*.18,.17,vWet);else if(organicSurface>0.5)roughnessFactor=mix(.68+skinNoise(vSkinPosition*.2)*.2,.13,vWet);
 `);
...
if(!document.documentElement.hasAttribute('data-model-gallery'))await Promise.all(['audio.js','models.js','boss-creatures.js','boss-machines.js','capital-ship.js','levels.js','runs.js','art.js','tide-encounter.js','game.js'].map(src=>new Promise((resolve,reject)=>{const s=document.createElement('script');s.async=false;s.src=src+new URL(import.meta.url).search;s.onload=resolve;s.onerror=reject;document.body.append(s)})));
