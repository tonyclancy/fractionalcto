...
if(renderer){
 ...
 const modelUniforms=m.userData.modelUniforms={motionTime:{value:0},rigKind:{value:0},hitFlash:{value:0},organicSurface:{value:organic?1:0},alienSurface:{value:0},pilotSurface:{value:0}};
 ...
 shader.fragmentShader=`varying float vTextureWeight; varying float vGlow; varying float vWet; varying vec3 vSkinPosition; uniform float alienSurface; uniform float pilotSurface; uniform float hitFlash; uniform float organicSurface;
 ...
 shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>', '#include <map_fragment>\n if(organicSurface>4.5){float relief=dot(diffuseColor.rgb,vec3(.2126,.7152,.0722));diffuseColor.rgb=vec3(.78+relief*.65);}else if(alienSurface>0.5){float relief=dot(diffuseColor.rgb,vec3(.2126,.7152,.0722));diffuseColor.rgb=mix(vec3(1.0),vec3(clamp(.32+relief*2.9,.28,1.35)),vTextureWeight);}');
 shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
 if(pilotSurface>0.5){diffuseColor.rgb*=.96+skinNoise(vSkinPosition*.15)*.04;}else if(alienSurface>0.5){float dermalPatch=skinNoise(vSkinPosition*.075);float pore=skinNoise(vSkinPosition*1.3);float fold=skinNoise(vSkinPosition*.38);float vein=1.0-smoothstep(.012,.05,abs(fold-.48));vec3 pigment=mix(vec3(.56,.70,.87),vec3(1.20,1.05,.78),smoothstep(.23,.76,dermalPatch));float scaleEdge=1.0-smoothstep(.06,.18,abs(sin(vSkinPosition.x*.72+sin(vSkinPosition.y*.9)*.7)*sin(vSkinPosition.z*.72+sin(vSkinPosition.y*.65)*.6)));pigment*=.83+pore*.25-vein*.12-scaleEdge*(alienSurface<1.5?.18:.07);diffuseColor.rgb*=mix(vec3(1.0),pigment,vTextureWeight*(1.0-vWet*.8));}else if(organicSurface>3.5){
 ...
 const organic=data.organic||rig==='squid'||rig==='octopus';s.surfaceKind=faces.rock?(faces.terrainMaterial==='ice'?4:faces.terrainMaterial==='storm'?5:faces.terrainMaterial?3:2):organic?1:0;s.alienKind=faces.alienMaterial==='chitin'?1:faces.alienMaterial==='flesh'?2:0;
 ...
 else{const m=s.normalMaterial;s.object.material=m;if(faces.terrainMaterial==='ice'&&!glacierTexture){glacierTexture=new THREE.TextureLoader().load('assets/terrain-glacier-v1.webp');glacierTexture.colorSpace=THREE.SRGBColorSpace;glacierTexture.wrapS=glacierTexture.wrapT=THREE.RepeatWrapping;glacierTexture.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());}const map=faces.alienMaterial?alienTextures[faces.alienMaterial]:faces.terrainMaterial==='ice'?glacierTexture:faces.rock?rockTexture:organic?skinColor:faces.industrial?industrialColor:null,bump=faces.terrainMaterial==='ice'?glacierTexture:faces.terrainMaterial==='storm'?rockTexture:faces.alienMaterial?alienTextures[faces.alienMaterial]:faces.rock?rockTexture:organic?skinTexture:faces.pilotHull?null:faces.capitalSurface||faces.industrial?industrialColor:metalTexture;if(m.map!==map||m.bumpMap!==bump){const programChanged=!!m.map!==!!map||!!m.bumpMap!==!!bump;m.map=map;m.bumpMap=bump;if(programChanged)m.needsUpdate=true;}m.metalness=faces.rock||organic?0:faces.pilotHull?.32:faces.capitalSurface?.58:faces.industrial?.3:.64;m.roughness=faces.terrainMaterial==='ice'?.72:faces.terrainMaterial==='reef'?.64:faces.rock?.98:faces.pilotHull?.57:faces.alienMaterial==='chitin'?.53:organic?.76:faces.capitalSurface?.46:faces.industrial?.8:.34;m.bumpScale=faces.alienMaterial==='chitin'?.95:faces.alienMaterial==='flesh'?.23:['ice','storm'].includes(faces.terrainMaterial)?.65:faces.rock?1.7:organic?1.1:.3;}
