...
function faunaJointVertex(p,age,joint){
 ...
 if(mode===9){
  ...
  const along=Math.max(0,(x-12)/55),u=Math.min(1,along),bend=u*u*(3-2*u);if(!bend)return p;
  const phase=age*6.8+Math.atan2(pz,py)*1.7+px*.07,lag=phase-along*5.2;
  const jet=Math.pow((1+Math.cos(age*4.8))*.5,4),tuck=organicTailTuck(age),gather=bend*(.26*jet+.58*tuck),free=1-.78*tuck;
  const twist=bend*Math.sin(lag*.73)*.75,c=Math.cos(twist),sn=Math.sin(twist),amplitude=bend*(11+5*u)*free;
  return[px+x-bend*(3+3*Math.sin(lag)),py+(y*c-z*sn)-p[1]*gather+amplitude*Math.sin(lag),pz+(y*sn+z*c)-p[2]*gather+amplitude*Math.cos(lag*.91+.8)];
 }
 ...
 else if(mode===7){const u=Math.max(0,Math.min(1,(Math.hypot(x,y,z)-12)/23)),bend=u*u*(3-2*u);if(!bend)return p;a=Math.sin(age*9-bend*3+px*.09+pz*.07)*bend*.34;axis='xz';}
...
(function buildConceptFauna(){
 ...
 function build(spec){
  ...
  if(tentacles||spec.form==='herald')for(let i=0;i<(spec.arms||6);i++){const a=i/(spec.arms||6)*Math.PI*2,y=Math.cos(a),z=Math.sin(a);const joint=[21,y*10,z*10,9];tube([[21,y*10,z*10],[39,y*17,z*16],[62,y*22,z*20],[82,y*16,z*24],[95,y*24,z*15]],2.6,tint(skin,1+i*.04),joint);for(let j=0;j<4;j++){const start=m.faces.length;m.ellipsoid(35+j*12,y*(15+j*2),z*(15+j*2)-.7,1.3,1,.8,tint(accent,.65),0,8,5);mark(start,joint);}}
...
function buildSpeciesAnatomy(spec){
 ...
 const arm=(root,points,r=2)=>tube([root,...points],r,skin,[...root,['squid','jelly'].includes(spec.anatomy)&&points.at(-1)[0]>root[0]?9:7]);
...
function buildSpeciesBossGeometry(spec,base){
...
// Size the solid body, not just a long tail or wing tip. A shared rigid scale
// keeps skin, joints, weapon sockets and collision volumes in agreement.
function buildSpeciesBoss(spec,base,progress=0){
 const d=buildSpeciesBossGeometry(spec,base),volumes=d.bodyVolumes;
 const bodyLength=Math.max(...volumes.map(v=>v.center[0]+v.radii[0]))-Math.min(...volumes.map(v=>v.center[0]-v.radii[0]));
 const lo=[Infinity,Infinity,Infinity],hi=[-Infinity,-Infinity,-Infinity];
 for(const f of d.mesh)for(const p of f.v)for(let axis=0;axis<3;axis++){lo[axis]=Math.min(lo[axis],p[axis]);hi[axis]=Math.max(hi[axis],p[axis]);}
 const span=hi.map((n,i)=>n-lo[i]),target=380+60*Math.max(0,Math.min(1,progress));
 // Leave navigation room even as a winged boss rolls its wings into view.
 const limit=Math.min(820/span[0],660/Math.max(span[1],span[2]));
 d.scale=Math.max(d.scale,Math.min(target/bodyLength,limit));
 d.footprint={bodyLength:bodyLength*d.scale,span:span.map(n=>n*d.scale)};
 return d;
}
...
function developSpeciesGenome(planetId,systemId,habitat,role,boss=false,climate='temperate',recipe=null){
 ...
 const anatomy=organicAnatomyProgram(genome);
 genome.locomotion=habitat==='water'?(['bell','squid','nautilus'].includes(anatomy)?'siphon':'fins'):['bell','squid','nautilus','leviathan','manta','sailback','hammerfin'].includes(anatomy)?'jets':'wings';
...
function organicAnatomyProgram(g){
 ...
 const castes=g.habitat==='water'?waterCastes[g.clade]||waterCastes.lancet:choices,role=g.boss?3:g.role===4?2:g.role===3?1:0;
 // Related ecologies need not put the same animal in the same encounter role.
 // Rotate the complete roster, preserving a separate leader and boss anatomy.
 const plan=castes[(role+((g.lineage>>>17)%castes.length))%castes.length];
 const relatives={drake:'sailwing',mantis:'razorcrab',manta:'hammerfin',leviathan:'sailback'};
 return relatives[plan]&&((g.lineage>>>4)&1)?relatives[plan]:plan;
}
function buildDevelopedOrganism(spec){
 ...
 const air=g.habitat==='air',boss=g.boss,bodyStretch=Math.max(.78,Math.min(1.30,g.bodyLength/80));
 // Express the genome in the silhouette, not merely names and slight recolouring.
 const girthScale=Math.max(.78,Math.min(1.35,g.girth/18)),depthScale=Math.max(.75,Math.min(1.3,g.depth/18));
 const soft=['bell','squid','manta','leviathan','hammerfin','sailback'].includes(plan),armored=!soft;
 ...
 const tube=(pts,r,c=skin,j=null,sides=8,steps=4)=>{const start=m.faces.length;m.tube(pts,r,c,0,0,sides,steps);mark(start,j);};
 ...
 const programs={
  sailwing:[[-48,0,1,3],[-35,-2,6,9],[-19,-3,12,17],[1,-1,16,21],[21,1,9,15],[38,4,3,5],[53,5,.4,.5]],
  razorcrab:[[-46,2,2,8],[-35,0,8,20],[-17,-3,17,28],[3,-3,19,27],[24,0,11,19],[39,3,.5,3]],
  hammerfin:[[-42,2,1,8],[-33,0,6,30],[-23,-1,8,32],[-9,0,12,19],[12,1,10,13],[34,3,4,7],[49,3,.5,1]],
  sailback:[[-52,1,1,3],[-40,0,6,8],[-26,-2,11,12],[-4,-4,16,15],[19,-2,12,12],[44,2,5,6],[71,4,.3,.5]],
 ...
 const profile=programs[plan].map((p,i,points)=>{const u=i/(points.length-1),head=i<3?1+(g.headSize-1)*(1-i/3)*.5:1;return[p[0]*bodyStretch+(i<3?(2-i)*(g.neck-10)*.35:0),p[1]+Math.sin(Math.PI*u)*g.arch*.25,p[2]*girthScale*head,p[3]*depthScale*head];});
 ...
 // Hinge sockets are seated below the tessellated skin, not tangent to it.
 function attachment(u,a,inset=2){const p=section(u);return surface(u,a,-Math.min(inset,Math.min(p[2],p[3])*.45));}
 function legs(count,reach=22){
  for(let i=0;i<count;i++)for(const side of [-1,1]){
   const root=attachment(.34+i*.32/Math.max(1,count-1),side*1.94,3),splay=side*(9+i*2),j=[...root,2],knee=[root[0]+3+i*2,root[1]+reach*.42,root[2]+splay],toe=[root[0]+reach*.5,root[1]+reach*.65,root[2]+splay*1.2];
   ell(root,[2.7,2.6,2.8],edge);tube([root,knee,toe,[toe[0]-4,toe[1]+2,toe[2]]],2.4,skin,j);ell(knee,[1.8,1.8,1.8],edge,j);
  }
 }
 function arms(count){
  for(let i=0;i<count;i++){
   // Negative X is the face/muzzle. Arms emerge from the rear shoulder and
   // trail positive X, so steering the actor also steers its whole anatomy.
   const a=i/count*Math.PI*2,p=surface(.75,a),len=(boss?68:59)*(1+(i%3)*.10),j=[...p,9],r=4.2;
   // Bury the sealed root inside the mantle, then emerge through the skin.
   // The twelve-unit animation collar stays fixed in both render paths.
   const buried=surface(.75,a,-r*1.4);buried[0]-=r*.6;
   const pts=[buried,p,[p[0]+len*.32,p[1]*1.14,p[2]*1.12],[p[0]+len*.7,p[1]*1.33+Math.sin(a)*5,p[2]*1.25],[p[0]+len,p[1]*.92+Math.sin(a)*7,p[2]*.88]];
   tube(pts,r,skin,j,10,10);
   armAttachments.push({pivot:p,mode:9,tip:pts.at(-1),rootRing:m.faces[m.faces.length-2].v,skinPoint:p});
   // Embedded suckers only on muscular arms, not rows of floating bones.
   for(let k=1;k<4;k++){const u=k/4,q=pts[2].map((v,n)=>v*(1-u)+pts[3][n]*u);ell([q[0],q[1]+1,q[2]], [1.2,.6,1.1],edge,j);}
  }
 }
 function claws(){for(const side of [-1,1]){
  const root=attachment(.33,side*1.98,6),j=[...root,2],elbow=[root[0]-10,root[1]+13,root[2]+side*11],palm=[root[0]-27,root[1]+17,root[2]+side*18];
 ...
 function wings(count=1,span=47){for(let pair=0;pair<count;pair++)for(const side of [-1,1]){
  const root=attachment(.36+pair*.18,side*1.18),spread=Math.min(78-Math.abs(root[2]),span*Math.max(.72,Math.min(1.3,g.wingSpan/54))*(pair?.72:1)),sweep=g.wingSweep*.65,chord=.75+(g.seed>>>12)%4*.18;
  membrane(root,[root[0]-(plan==='moth'?24:10)+sweep,root[1]-4,side*(Math.abs(root[2])+spread)],[root[0]+(plan==='moth'?23:36)*chord+sweep,root[1]+5,side*(Math.abs(root[2])+spread*.66)]);
 }}
 ...
 if(plan==='sailwing'){wings(2,65);legs(1,17);scutes(3);}
 if(plan==='razorcrab'){wings(1,34);claws();legs(2,16);scutes(5);}
 if(plan==='hammerfin'){for(const side of [-1,1]){const root=attachment(.55,side*1.5);membrane(root,[root[0]+9,root[1]+6,side*65],[root[0]+38,root[1]+9,side*30],3);}scutes(3);}
 if(plan==='sailback'){for(const side of [-1,1]){const root=attachment(.43,side*1.6);membrane(root,[root[0]+4,root[1]+9,side*31],[root[0]+28,root[1]+5,side*18],3);}const root=attachment(.35,0);membrane(root,[root[0]+13,root[1]-39,2],[root[0]+58,root[1]-5,1],4);scutes(4);}
 ...
 if(plan==='manta'){wings(1,64);const p=attachment(.94,0);tube([p,[p[0]+14,p[1]+4,0],[p[0]+35,p[1]+6,3],[p[0]+48,p[1],4]],2.1,skin,[...p,4]);}
 if(plan==='leviathan'){
  for(const side of [-1,1]){const root=attachment(.37,side*1.65);membrane(root,[root[0]+4,root[1]+16,side*39],[root[0]+31,root[1]+10,side*23],3);}
 ...
 if(plan==='squid'){arms(6);for(const side of [-1,1]){const root=attachment(.56,side*1.55);membrane(root,[root[0]+12,root[1]-4,side*36],[root[0]+35,root[1]+4,side*12],3);}}
 if(plan==='nautilus'){arms(4);scutes(5);for(const side of [-1,1]){const pts=Array.from({length:30},(_,i)=>{const a=i/29*Math.PI*3.8,r=4+i*.64;return[7+Math.cos(a)*r,-5+Math.sin(a)*r,side*(24-i*.12)];});tube(pts,1.3,edge);}}
 if(plan==='crab'){legs(3,19);claws();scutes(2);if(air)wings(2,57);}
 if(plan==='trilobite'){scutes(7);for(let pair=0;pair<3;pair++)for(const side of [-1,1]){const root=attachment(.36+pair*.13,side*1.85);membrane(root,[root[0]-6,root[1]+7,side*(Math.abs(root[2])+15)],[root[0]+11,root[1]+4,side*(Math.abs(root[2])+12)],air?1:3);}}
 if(air&&['bell','squid','nautilus','leviathan','manta','sailback','hammerfin'].includes(plan)){
 ...
 if(armored&&plan!=='leviathan')for(const side of [-1,1]){const root=attachment(.20,side*.65);tube([root,[root[0]-8,root[1]-7,root[2]+side*5],[root[0]-18,root[1]-8,root[2]+side*8]],.8,edge,[...root,7]);}
