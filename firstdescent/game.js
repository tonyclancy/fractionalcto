...
function updateBossWeapon(b,dt){
 ...
 hostile.push({x:r.muzzleX,y:r.muzzleY,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,r:bossIndex()===0?12:7,c:sectors[level].color,kind:r.organic?organicShotKind():'bolt',launchAngle:angle,scale:bossIndex()===0?1.55:1});
 b.muzzle=.16;window.flightAudio?.shot(bossIndex()===1?'bolt':'spore',b.x,true);a.index++;a.next+=interval;
 if(a.index>=count){b.attack=null;b.fireHeading=null;b.shoot=b.hp/b.max<.5?1.15:1.8}
}
...
if(pointer){const px=(pointer.x-ship.x)*12,py=(pointer.y-ship.y)*12,scale=Math.min(1,maxSpeed/(Math.hypot(px,py)||1));targetVX=px*scale;targetVY=py*scale}
const response=1-Math.exp(-dt*(dx||dy||pointer?20:28));ship.vx+=(targetVX-ship.vx)*response;ship.vy+=(targetVY-ship.vy)*response;
const suction=boss?bossSuctionForce(boss):0;ship.x=clamp(ship.x+(ship.vx+suction)*dt,40,W-65);ship.y=clamp(ship.y+(ship.vy+sectorCurrent())*dt,42,H-42);if(ship.x===40||ship.x===W-65)ship.vx=0;if(ship.y===42||ship.y===H-42)ship.vy=0;
...
window.flightAudio?.setIntensity(boss?.9:Math.min(.75,enemies.length/24));if(boss){const b=boss;b.age+=dt;b.depth=1;b.muzzle=Math.max(0,(b.muzzle||0)-dt);updateBossSpecial(b,dt);updateEncounter(b,dt);updateBossArms(b,dt);moveBoss(b,dt);b.hit=Math.max(0,b.hit-dt);updateBossWeapon(b,dt);if(bossBodyHit(b,ship.x,ship.y,18))damage()}
...
if(boss&&boss.hp<=0){window.flightAudio?.clear();explode(boss.x,boss.y,sectors[level].color,3.8,bossOrganic(),64+bossIndex());hazards=[];acidClouds=[];rings.push({x:boss.x,y:boss.y,r:20,life:1.1,c:'#fff'});score+=3000*(level+1);boss=null;bossDefeated=true;transition=4;hostile=[];shots=[];flash=.65;shake=20;$('#bossbar').hidden=true;announce('SECTOR CLEARED','JUMP DRIVE CHARGING');tone(55,.9,'triangle',.04,-25)}
...
function setPointer(e){const r=canvas.getBoundingClientRect();pointer={x:(e.clientX-r.left)/r.width*W,y:(e.clientY-r.top)/r.height*H}}
canvas.addEventListener('pointerdown',e=>{if(state!=='playing')return;canvas.setPointerCapture(e.pointerId);setPointer(e)});canvas.addEventListener('pointermove',e=>{if(pointer)setPointer(e)});canvas.addEventListener('pointerup',()=>pointer=null);canvas.addEventListener('pointercancel',()=>pointer=null);
