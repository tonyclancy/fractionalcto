...
function universeCatalog(){return [...expeditionSystems.values()].map(system=>({system,galaxy:GALAXIES[system.galaxyId],worlds:system.destinations.filter(d=>d.kind==='planet')}));}
function showUniverseAtlas(selectedId=null){
 ...
 const counts=`${new Set(catalog.map(e=>e.galaxy.id)).size} GALAXIES · ${catalog.length} SUNS · ${catalog.reduce((sum,e)=>sum+e.worlds.length,0)} WORLDS`;
 overlay.className='overlay universe-atlas';overlay.innerHTML=`<div class="atlas-shell"><header class="atlas-header"><div><span class="eyebrow mint">EXPEDITION ATLAS</span><h2>${entry?entry.system.name+' SYSTEM':'THE KNOWN UNIVERSE'}</h2><p>${entry?entry.galaxy.name+' · '+systemCensusLabel(entry.system):counts}</p></div><button id="atlasBack">${entry?'← ALL GALAXIES':'← MAIN MENU'}</button></header>${entry?`<div class="atlas-detail"><canvas id="atlasMap" width="800" height="550" aria-label="${entry.system.name} orbital map"></canvas><div class="atlas-worlds"><p class="atlas-note">Conquest route · one descent per world</p>${entry.worlds.map((d,i)=>{const stage=campaign.find(s=>s.id===d.stages[0]);return `<article><span class="atlas-number">${String(i+1).padStart(2,'0')}</span><div><h3>${d.name}${d.rings?' <small>RINGED</small>':''}</h3><p>${ENVIRONMENTS[stage.environment]?.label||stage.environment} · ${d.climate.toUpperCase()} · ${d.orbit} AU</p><p>${stage.name}</p><p class="atlas-note">NATIVE LIFE · ${stage.biosphere.names.join(" · ")}</p></div></article>`;}).join('')}<p class="atlas-note">${entry.system.star.name} · ${entry.system.star.type}<br>Hot inner orbits → cold outer worlds.</p></div></div>`:`<p class="atlas-note">Begin in Vesper. Conquer its three planets, then cross to the next galaxy. Select a galaxy to explore its worlds.</p><div class="atlas-grid">${catalog.map((e,i)=>`<button class="atlas-card" data-system="${e.system.id}" style="--galaxy:url('assets/${galaxyImageFile(e.galaxy)}');--galaxy-tint:${e.galaxy.tint.join(',')}"><span class="atlas-number">${String(i+1).padStart(2,'0')}</span><strong>${e.galaxy.name}</strong><span>${e.system.name} · ${e.worlds.length} WORLDS</span><small>${e.worlds.map(d=>d.name).join(' · ')}</small></button>`).join('')}</div>`}</div>`;
...
function beginDescent(){atlasOpen=false;atlasSystemId=null;$('#overlay').onclick=null;enableAudio();const launch=()=>{start();sectorBlend={arrival:true,systemEntry:true,age:0,duration:15.5,destination:expedition.locations[sectors[0].id]};$('#announcement').style.opacity=0;annTimer=0;};if(window.safariImmersion?.request(launch))return;void ensureGameFullscreen();launch();}
...
function testLevelHotkey(e){
 ...
 const k=e.key.toLowerCase(),direct=/^[1-9]$/.test(k),travel=k==='n',anomaly=k==='b';
 if(!direct&&k!=='['&&k!==']'&&!travel&&!anomaly)return false;
 e.preventDefault();if(e.repeat)return true;
 const target=anomaly?sectors.findIndex(s=>s.gravityWell):direct?Number(k)-1:level+(k==='['?-1:1);
 ...
 if(target!==level){advanceSector(target);sectorBlend.testing=true;}else{resetSector();sectorBlend={arrival:true,systemEntry:true,testing:true,age:0,duration:15.5,destination:expedition.locations[sectors[level].id]};$('#announcement').style.opacity=0;annTimer=0;}
