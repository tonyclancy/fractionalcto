...
function showTitleScreen(){atlasOpen=false;atlasSystemId=null;$('#overlay').onclick=null;state='title';$('#overlay').className='overlay title-screen';$('#overlay').innerHTML=titleMarkup;$('#launch').onclick=beginDescent;bindUniverseButton();const music=$('#introMusic');if(music)music.onclick=()=>$('#music').onclick();window.flightAudio?.setTitle(!document.hidden);musicControls();updateExpeditionIntro();drawNavigationChart();window.refreshCollectionButtons?.();}
let atlasSystemId=null,atlasOpen=false;
function bindUniverseButton(){const mission=$('#mission');if(mission)mission.onclick=showOriginMission;const button=$('#universe');if(button)button.onclick=()=>showUniverseAtlas();}
...
function galaxyAttribution(galaxy){return '<p class="atlas-credit"><strong>'+galaxy.name+'</strong> · <a href="'+galaxy.source+'" target="_blank" rel="noopener">'+galaxy.reference+'</a><br>'+galaxy.credit+'</p>';}
function showUniverseAtlas(selectedId=null){
 ...
 overlay.className='overlay universe-atlas';overlay.innerHTML=`<div class="atlas-shell"><header class="atlas-header"><div><span class="eyebrow mint">EXPEDITION ATLAS</span><h2>${entry?entry.system.name+' SYSTEM':'THE KNOWN UNIVERSE'}</h2><p>${entry?entry.galaxy.name+' · '+systemCensusLabel(entry.system):counts}</p></div><button id="atlasBack">${entry?'← ALL GALAXIES':'← MAIN MENU'}</button></header>${entry?`<div class="atlas-detail"><canvas id="atlasMap" width="800" height="550" aria-label="${entry.system.name} orbital map"></canvas><div class="atlas-worlds">${galaxyAttribution(entry.galaxy)}<p class="atlas-note">Conquest route · one encounter per destination</p>${entry.worlds.map((d,i)=>{const stage=campaign.find(s=>s.id===d.stages[0]);return `<article><span class="atlas-number">${String(i+1).padStart(2,'0')}</span><div><h3>${d.name}${d.rings?' <small>RINGED</small>':''}</h3><p>${ENVIRONMENTS[stage.environment]?.label||stage.environment} · ${d.stellarBody?.type||d.climate.toUpperCase()} · ${d.stellarBody?'STELLAR ORBIT':d.orbit+' AU'}</p><p>${stage.name}</p><p class="atlas-note">NATIVE LIFE · ${stage.biosphere.names.join(" · ")}</p></div></article>`;}).join('')}<p class="atlas-note">${systemFocus(entry.system).name} · ${systemFocus(entry.system).type}<br>${entry.system.centralBody?systemCensus(entry.system).suns+' stellar coronas · no planetary landings.':'Hot inner orbits → cold outer worlds.'}</p></div></div>`:`<p class="atlas-note">Begin in Vesper. Conquer its three planets, then cross to the next galaxy. Select a galaxy to explore its worlds.</p><div class="atlas-grid">${catalog.map((e,i)=>`<button class="atlas-card" data-system="${e.system.id}"><img class="atlas-galaxy" style="${galaxyThumbnailMask(e.galaxy)}" src="assets/${galaxyThumbnailFile(e.galaxy)}" alt="${e.galaxy.morphology} galaxy inspired by ${e.galaxy.reference}" loading="lazy" decoding="async" width="480" height="300"><span class="atlas-number">${String(i+1).padStart(2,'0')}</span><span class="atlas-kind">${e.galaxy.morphology.replaceAll('-',' ').toUpperCase()}</span><strong>${e.galaxy.name}</strong><span>${e.system.name} · ${systemCensusLabel(e.system)}</span><small>${e.worlds.map(d=>d.name).join(' · ')}</small></button>`).join('')}</div><section class="atlas-credits"><h3>THE REAL UNIVERSE BEHIND THE JOURNEY</h3><p>Fictional destinations, grounded in observed galaxy forms. Telescope colours represent the captured wavelengths; rotation and travel time are compressed for the game. Images resized and softly blended into the starfield.</p>${catalog.map(e=>galaxyAttribution(e.galaxy)).join('')}<p>Observation images used under <a href="https://creativecommons.org/licenses/by/4.0/" target="_blank" rel="noopener">CC BY 4.0</a>. No endorsement is implied.</p></section>`}</div>`;
...
// Mission presentation has no effect on score, damage or scheduled supplies.
const ORIGIN_RECOVERY_DURATION=6.8;
let originRecovery=null,originFinalePending=false;
function originEntry(id){for(const s of ORIGIN_EXPEDITION.systems){const e=s.stages.find(e=>e.id===id);if(e)return e;}return null;}
function originFragmentName(kind){return kind==='core'?'MEMORY CORE':kind==='ember'?'CAPTIVE EMBER':'LUMINOUS SEED';}
function missionObjective(){const p=originStore.snapshot();return p.complete?'ORIGIN RESTORED':p.unlocked?'ORIGIN GATE OPEN':`SIGNAL · STAGE ${p.milestone.index} · ${p.milestone.count} / ${p.milestone.total}`;}
function updateOriginPresentation(){
 const p=originStore.snapshot(),hud=$('#missionHud');if(hud&&hud.textContent!==missionObjective())hud.textContent=missionObjective();
 window.flightAudio?.setSignalProgress?.(p.count/p.total);
}
function showOriginMission(){
 if(state!=='title')return;updateOriginPresentation();atlasOpen=true;atlasSystemId=null;const p=originStore.snapshot(),next=originEntry(p.next),overlay=$('#overlay');
 overlay.className='overlay universe-atlas origin-log';
 overlay.innerHTML=`<div class="atlas-shell"><header class="atlas-header"><div><span class="eyebrow mint">EXPEDITION 001 · PERMANENT DISCOVERIES</span><h2>THE ORIGIN SIGNAL</h2><p>Follow the lost expedition. Recover the memories held by its guardians.</p></div><button id="originBack">← MAIN MENU</button></header><section class="origin-summary"><div><span class="eyebrow">${p.complete?'ARCHIVE RESTORED':'STAGE '+p.milestone.index+' · '+p.milestone.next.name}</span><strong>${p.complete?p.count:p.milestone.count}<small> / ${p.complete?p.total:p.milestone.total} memories</small></strong><progress value="${p.complete?p.count:p.milestone.count}" max="${p.complete?p.total:p.milestone.total}" aria-label="Current mission stage progress"></progress></div><div><h3>${p.complete?'You found their voices.':p.unlocked?'The Origin guardian awaits.':'Next: '+next.name}</h3><p>${p.complete?'The expedition was never silent. Its memories are safe, and a route home shines across the stars.':p.unlocked?'The assembled signal has opened the route to Aureus, in Eventide. Defeat its final guardian to restore the archive.':'Each guardian holds one unique memory. Complete this short stage to unlock a pilot title and a decoded expedition memory. Follow the signal launches a fresh ship at your next missing fragment.'}</p><button class="primary" id="originContinue">${p.complete?'FLY AGAIN':'FOLLOW THE SIGNAL'} <span>↗</span></button></div></section><p class="atlas-note">${p.durable?'Saved on this device.':'Progress lasts for this session; device storage is unavailable.'} Fragments survive defeat. Replays do not award duplicates. Future expeditions have separate collections.</p><section class="origin-milestones"><h3>${p.milestone.previous?'PILOT TITLE · '+p.milestone.previous.title:'YOUR FIRST REWARD · SIGNAL SCOUT'}</h3><p>${p.milestone.next?'Next reward: '+p.milestone.next.title+' · '+(p.milestone.total-p.milestone.count)+' more memories.':'Every stage complete.'} Stage goals stay fixed as new expeditions arrive.</p><div class="milestone-list">${ORIGIN_EXPEDITION.milestones.map((m,i)=>{const done=p.count>=m.target;return `<article class="${done?'earned':''}"><small>STAGE ${i+1} · ${m.target} TOTAL</small><strong>${done?'✦':'◇'} ${m.name}</strong><span>${m.title}</span><p>${done?m.memory:'Recover this stage’s memories to decode the next chapter.'}</p></article>`}).join('')}</div></section><details class="origin-archive"><summary>FULL EXPEDITION ARCHIVE · ${p.count} / ${p.total} · ${ORIGIN_EXPEDITION.systems.length} CONSTELLATIONS</summary><div class="origin-grid">${ORIGIN_EXPEDITION.systems.map((s,i)=>{const status=p.systems[i],done=status.count===status.total;return `<article class="origin-system ${done?'resolved':''}"><div class="origin-system-title"><h3>${s.name}</h3><span>${status.count} / ${status.total}</span></div><svg viewBox="0 0 280 70" role="img" aria-label="${s.name}: ${status.count} of ${status.total} fragments"><path d="${s.stages.map((e,j)=>(j?'L':'M')+(25+j*230/Math.max(1,s.stages.length-1))+','+(j%2?48:22)).join(' ')}"/><g>${s.stages.map((e,j)=>`<circle cx="${25+j*230/Math.max(1,s.stages.length-1)}" cy="${j%2?48:22}" r="${originStore.has(e.id)?7:4}" class="${originStore.has(e.id)?'recovered':''}"/>`).join('')}</g></svg><ul>${s.stages.map(e=>`<li><span class="origin-mark ${originStore.has(e.id)?'recovered':''}" aria-label="${originStore.has(e.id)?'Recovered':'Missing'}">${originStore.has(e.id)?'✦':'◇'}</span> ${e.name}<small>${originFragmentName(e.kind)}</small></li>`).join('')}</ul><p class="origin-discovery">${done?s.discovery:'Complete this constellation to decode its memory.'}</p></article>`;}).join('')}</div></details></div>`;
 overlay.scrollTop=0;overlay.scrollIntoView?.({block:'start'});$('#originBack').onclick=closeUniverseAtlas;$('#originBack').focus({preventScroll:true});
 $('#originContinue').onclick=()=>beginDescent(p.complete?0:Math.max(0,sectors.findIndex(s=>s.id===p.next)));
}
function recoverOriginFragment(b){
 if(!flightRun||b!==boss||b.hp>0)return null;const recovered=originStore.collect(sectors[level].id);if(!recovered)return null;
 const progress=originStore.snapshot();
 originRecovery={x:clamp(b.x,70,W-70),y:clamp(b.y,90,H-90),kind:recovered.entry.kind,age:0,recovered,progress,phase:-1};originFinalePending=recovered.complete;
 updateOriginPresentation();updateOriginRecovery(0);
 return recovered;
}
function updateOriginRecovery(dt){
 if(!originRecovery)return;const r=originRecovery;r.age+=dt;
 if(r.age>=ORIGIN_RECOVERY_DURATION){originRecovery=null;const panel=$('#originRecoveryPanel');if(panel)panel.hidden=true;return;}
 const phase=r.age<1.5?0:r.age<4.1?1:2;
 if(phase===r.phase)return;r.phase=phase;
 if(phase===2)window.flightAudio?.signalRecovered?.(r.progress.count/r.progress.total,r.recovered.completeSystem||!!r.recovered.milestone);
 const panel=$('#originRecoveryPanel');if(!panel)return;
 const name=originFragmentName(r.kind),description=r.kind==='core'?'Recovered from the guardian’s wreck.':r.kind==='ember'?'Released from the guardian’s stellar heart.':'Released from the guardian’s living core.';
 const milestone=r.recovered.milestone,stage=r.progress.milestone,stageCount=milestone?milestone.target-(ORIGIN_EXPEDITION.milestones[stage.earned.length-2]?.target||0):stage.count,stageTotal=milestone?stageCount:stage.total;
 const system=r.progress.systems.find(s=>s.id===r.recovered.system.id),count=system?.count??0,total=system?.total??r.recovered.system.stages.length;
 panel.hidden=false;panel.innerHTML=`<span class="eyebrow">${phase===0?'GUARDIAN DEFEATED · SIGNAL DETECTED':phase===1?'RECOVERY LINK · DRAWING FRAGMENT ABOARD':milestone?'STAGE COMPLETE · REWARD UNLOCKED':'FRAGMENT SECURED · ORIGIN SIGNAL'}</span><strong>${name}</strong><p>${description} A piece of the lost expedition’s memory.</p><div class="recovery-progress"><b>${stageCount} / ${stageTotal}</b><span>${milestone?milestone.name:stage.next.name}</span><span>${r.recovered.system.name} · ${count} / ${total}</span></div><div class="recovery-track"><i style="width:${stageCount/Math.max(1,stageTotal)*100}%"></i></div><p class="recovery-purpose">${milestone?'Pilot title: '+milestone.title+'. '+milestone.memory:r.recovered.complete?'The archive is restored. You have found the expedition’s route home.':r.recovered.completeSystem?r.recovered.system.discovery:'Collect the guardians’ memories to reveal the Origin and find the route home.'}</p><small>${phase===2?(r.progress.durable?'Saved on this device · Survives defeat':'Recovered for this session · Device saving unavailable'):'Recovering automatically · Keep flying'}</small>`;
}
function drawOriginRecovery(){
 if(!originRecovery)return;const r=originRecovery,travel=clamp((r.age-1.5)/2.6,0,1),ease=travel*travel*(3-2*travel),x=r.x+(ship.x-r.x)*ease,y=r.y+(ship.y-r.y)*ease-Math.sin(travel*Math.PI)*100,c=r.kind==='core'?'#8bcfff':r.kind==='ember'?'#ffb66c':'#a4ffe0',fade=1-clamp((r.age-4.1)/.85,0,1),size=(26+Math.sin(r.age*3)*2)*(1-.76*ease);
 ctx.save();ctx.globalAlpha=Math.min(1,r.age*2)*fade;
 // A short stream joins the source to the ship while the fragment is drawn in.
 if(travel>0&&travel<1){ctx.strokeStyle=c;ctx.lineWidth=1;ctx.globalAlpha*=.32;ctx.beginPath();ctx.moveTo(x,y);ctx.quadraticCurveTo((x+ship.x)/2,Math.min(y,ship.y)-55,ship.x,ship.y);ctx.stroke();ctx.globalAlpha=Math.min(1,r.age*2)*fade;}
 for(let i=0;i<12;i++){const a=i*TAU/12+r.age*(i%2?1:-1),radius=(35+i%3*8)*(1-ease),px=x+Math.cos(a)*radius,py=y+Math.sin(a)*radius*.65;ctx.fillStyle=i%3?c:'#f1ffff';ctx.fillRect(px,py,1.5+i%2,1.5+i%2);}
 ctx.translate(x,y);const light=ctx.createRadialGradient(0,0,0,0,0,size*3);light.addColorStop(0,c+'aa');light.addColorStop(.35,c+'35');light.addColorStop(1,c+'00');ctx.fillStyle=light;ctx.fillRect(-size*3,-size*3,size*6,size*6);
 ctx.rotate(r.age*.65);ctx.strokeStyle=c;ctx.lineWidth=1.5;
 const shape=ctx.createLinearGradient(-size,-size,size,size);shape.addColorStop(0,'#efffff');shape.addColorStop(.42,c);shape.addColorStop(1,r.kind==='ember'?'#783422':'#1a596c');ctx.fillStyle=shape;
 ctx.beginPath();const sides=r.kind==='core'?6:5;for(let i=0;i<sides;i++){const a=i*TAU/sides,px=Math.cos(a)*size*(r.kind==='seed'?.65:1),py=Math.sin(a)*size;i?ctx.lineTo(px,py):ctx.moveTo(px,py);}ctx.closePath();ctx.fill();ctx.stroke();ctx.strokeStyle='#efffff';ctx.beginPath();ctx.moveTo(-size*.35,-size*.5);ctx.lineTo(0,0);ctx.lineTo(size*.35,size*.45);ctx.stroke();ctx.restore();
 if(r.age>=4.1&&r.age<4.8){ctx.save();ctx.globalAlpha=(1-(r.age-4.1)/.7)*.5;const g=ctx.createRadialGradient(ship.x,ship.y,0,ship.x,ship.y,65);g.addColorStop(0,c);g.addColorStop(1,c+'00');ctx.fillStyle=g;ctx.fillRect(ship.x-65,ship.y-65,130,130);ctx.restore();}
}
...
let state='title',level=0,time=0,world=0,last=0,spawnClock=0,score=0,weapon='pulse',power=1,novas=3,fireClock=0,shake=0,flash=0,boss=null,bossDefeated=false,transition=0,kills=0,annTimer=0,annPriority=0,sound=true;
...
// View the interceptor from its flank, with a small view of the dorsal armor.
// Movement rolls are relative to this resting attitude, so letting go returns
// to a side silhouette. The coupled half-turn keeps the canopy above the hull
// in either direction; mounts and exhaust use this exact same pose.
const PILOT_SIDE_ROLL=-1.2;
function pilotFlightPose(){return{yaw:flightPose.yaw+pilotTurn.angle,roll:PILOT_SIDE_ROLL+flightPose.roll+pilotTurn.angle,pitch:flightPose.pitch};}
function pilotMount(x,y=0,z=0){const pose=pilotFlightPose(),p=projectPilotHull([x,y,z],pose.yaw,pose.roll,pose.pitch);return{x:ship.x+p.x,y:ship.y+p.y};}
...
function refreshHullMeter(){
 ...
 const hp=clamp(ship?.hp||0,0,5);hudWidth('#hullMeter',(hp*20)+'%');if(meter.parentElement?.getAttribute('aria-valuenow')!==String(hp))meter.parentElement?.setAttribute('aria-valuenow',String(hp));hudText('#hullReadout',hp+' / 5'+(rescueCharge?' · RESCUE READY':''));if(block.classList.contains?.('low-hull')!==(hp<=2))block.classList.toggle?.('low-hull',hp<=2);
}
...
function burst(x,y,color,count=22){for(let i=0;i<count;i++){const a=rand(0,TAU),v=rand(45,320);particles.push({x,y,vx:Math.cos(a)*v,vy:Math.sin(a)*v,life:rand(.2,.8),max:.8,c:color,r:rand(1,4)})}if(sectors[level].medium!=='water')rings.push({x,y,r:8,life:.5,c:color})}
function announce(title,sub='',priority=boss?2:1){
 if(annTimer>0&&priority<annPriority)return false;
 const el=$('#announcement');el.textContent=title;if(sub){const small=document.createElement('small');small.textContent=sub;el.append(small)}
 el.className=/^SECTOR|^FIRST CONTACT/.test(title)?'':'compact';el.style.opacity=1;annPriority=priority;annTimer=/^SECTOR/.test(title)?2.4:1.8;return true;
}
// Retain references to the permanent HUD, and write only changed values. A
// replaced element is discovered automatically; no display update is throttled.
const hudElements=new Map();let sectorHUDCache=null;
function hudElement(id){let e=hudElements.get(id);if(!e||e.isConnected===false){e=$(id);if(e)hudElements.set(id,e);}return e;}
function hudText(id,value){const e=hudElement(id);if(e&&e.textContent!==value)e.textContent=value;}
function hudWidth(id,value){const e=hudElement(id);if(e&&e.style.width!==value)e.style.width=value;}
function sectorHUDInfo(){
 const stage=sectors[level],location=expedition.locations[stage.id];
 if(sectorHUDCache?.stage===stage&&sectorHUDCache.location===location)return sectorHUDCache;
 const progress=systemStageProgress(location),planets=expeditionSystem(location).destinations.filter(d=>d.kind==='planet');
 return sectorHUDCache={stage,location,number:`${String(progress.number).padStart(2,'0')} / ${String(progress.total).padStart(2,'0')}`,name:location.destinationName+' · '+stage.short,destination:location.destinationKind==='star'?`${location.systemName} · STELLAR CORONA`:`${location.galaxyName} · ${location.systemName} · PLANET ${planets.findIndex(d=>d.id===location.destinationId)+1} / ${planets.length}`};
}
function updateHUD(){updateOriginPresentation();hudText('#score',String(score).padStart(6,'0'));const info=sectorHUDInfo();hudText('#sectorNum',info.number);hudText('#sectorName',info.name);hudText('#destinationProgress',info.destination);hudText('#hearts','▰ '.repeat(Math.max(0,ship.hp))+'▱ '.repeat(5-Math.max(0,ship.hp)));hudText('#weapon',`${weaponNames[weapon]} / MK ${['I','II','III'][power-1]}${ship.shield>0?' + SHIELD':''}`);hudText('#loadout',`BOOST ${speedLevel}/3 · DRONES ${companion}/2${ship.frontShield?' · GUARD '+ship.frontShield:''}${weaponOrb.owned?' · ORB GUARD':''} · FACING ${shipDirection()===1?'RIGHT':'LEFT'}`);hudText('#novas','● '.repeat(novas)+'○ '.repeat(3-novas));hudText('#progressLabel',`${!flightRun&&state!=='title'?'TEST FLIGHT · ':''}${sectors[level].scrollAxis==='down'?'DESCENT · ':sectors[level].scrollAxis==='up'?'ASCENT · ':''}SECTION ${currentSection()+1} / 4`);hudWidth('#progress',Math.min(100,time/sectors[level].duration*100)+'%');hudText('#status',state==='playing'?(boss?'BOSS ENGAGED':`SECTION ${currentSection()+1} / 4`):state==='title'?'AWAITING PILOT':state.toUpperCase());if(boss){hudWidth('#bosshealth',Math.max(0,boss.hp/boss.max*100)+'%');const hint=$('#bosstactic');if(hint)hudText('#bosstactic',bossEncounterProfile(sectors[level]).signature+' · '+(typeof isCapitalSiege==='function'&&isCapitalSiege(boss)?capitalSiegeHint(boss):typeof bossEncounterHint==='function'?bossEncounterHint(boss):boss.exposed>0?'EXPOSED · ATTACK NOW':encounterRules[sectors[level].bossKind].hint));}refreshHullMeter();}
...
function end(win){if(!win&&flightRun)flightRun.deaths++;recordFlightRun(win);window.flightAudio?.setMusicActive(false);window.flightAudio?.clear();state=win?'victory':'gameover';$('#pause').hidden=true;$('#touchControls').classList.remove('active');$('#bossbar').hidden=true;
 if(win&&originStore.snapshot().complete){panel('ORIGIN<br><em>RESTORED</em>','The signal resolves into voices. The lost expedition lives on inside the archive.<br>You have opened their route home.<br>All '+originStore.snapshot().total+' fragments recovered. Expedition 001 complete.','MISSION LOG',()=>{showTitleScreen();showOriginMission();});updateHUD();return;}
 panel(win?'SYSTEMS<br><em>SECURED</em>':'SIGNAL<br><em>LOST</em>',win?`Flight complete.<br>Origin Signal: ${originStore.snapshot().count} / ${originStore.snapshot().total} fragments recovered. Open the mission log to follow the remaining signal.`:`Continue ${expedition.locations[sectors[level].id].destinationName}, section ${checkpoint.section+1} of 4.<br>Your checkpoint loadout returns, with power and shield pickups ahead.`,win?'FLY AGAIN':'CONTINUE DESCENT',win?beginDescent:retrySection);updateHUD()}
...
function collect(d){
 ...
 score+=50;burst(d.x,d.y,'#a8ffdb',12);window.flightAudio?.pickup(d.x,d.type);
 ...
 if(announce(label,'',0))annTimer=1.4;updateHUD();return true;
}
...
function advanceSector(target=level+1){
 ...
 if(flightRun&&sectors[target].id===ORIGIN_EXPEDITION.finalStage&&!originStore.snapshot().unlocked)target=sectors.findIndex(s=>s.id===originStore.snapshot().next);
 ...
 level=target;ship.hp=Math.min(5,ship.hp+2);novas=Math.min(3,novas+1);resetSector(true);Object.assign(ship,motion);
...
if(state!=='playing')return;if(bossDefeated){transition-=dt;if(transition<=0){if(originFinalePending||level===sectors.length-1){end(true)}else{advanceSector()}}}hudClock+=dt;if(hudClock>=.08){hudClock=0;updateHUD()}}
...
function musicControls(){const info=window.flightAudio?.status?.()||window.flightAudio?.stats();if(!info)return;const button=$('#music');const label=!info.musicEnabled?'MUSIC OFF':info.musicPlaying||state!=='title'?'MUSIC ON':'PLAY MUSIC';if(button.textContent!==label){button.textContent=label;button.setAttribute('aria-label',info.musicEnabled&&(info.musicPlaying||state!=='title')?'Turn off music':'Play music');button.setAttribute('aria-pressed',String(info.musicEnabled));}const titleButton=$('#introMusic'),titleLabel=$('#introMusicLabel');if(titleButton&&titleLabel){const playing=info.musicEnabled&&info.musicPlaying,label=playing?'PAUSE THE THEME':'PLAY THE THEME';if(titleLabel.textContent===label&&titleButton.flightMusicPlaying===playing)return;titleButton.flightMusicPlaying=playing;titleLabel.textContent=label;titleButton.setAttribute('aria-label',playing?'Pause the theme':'Play the theme');titleButton.setAttribute('aria-pressed',String(playing));titleButton.classList.toggle('playing',playing);}}
$('#music').onclick=()=>{const info=window.flightAudio?.status?.()||window.flightAudio?.stats();if(!info)return;window.flightAudio.setMusicEnabled(!info.musicEnabled||(state==='title'&&info.state!=='running'));if(state==='title')window.flightAudio.setTitle(!document.hidden);else window.flightAudio.setMusicActive(state==='playing'&&!document.hidden);enableAudio();musicControls()};
...
function recoverGestureAudio(event){
 ...
 const info=window.flightAudio?.status?.()||window.flightAudio?.stats();
