...
function advanceSector(){
 ...
 const destination=expedition.locations[sectors[level].id],travel=destination?.destinationId!==fromDestination;sectorBlend={outgoing,haze,age:0,duration:destination.galaxyId!==fromLocation.galaxyId?15:destination.systemId!==fromLocation.systemId?10.2:travel?8.4:2.2,origin:fromLocation,galaxyEntry:destination.galaxyId!==fromLocation.galaxyId,systemEntry:destination.systemId!==fromLocation.systemId,destination:travel?destination:null};$('#announcement').style.opacity=0;annTimer=0;window.flightAudio?.setIntensity(0);
}
...
function render(dt){resizeFlightSurface();ctx.setTransform(renderScale,0,0,renderScale,0,0);window.gpuModels?.begin();ctx.save();if(shake)ctx.translate(rand(-shake,shake),rand(-shake,shake));const navigationOpaque=sectorBlend?.galaxyEntry&&sectorBlend.age/sectorBlend.duration<.936;if(!navigationOpaque){background(dt);drawDreamAtmosphere();drawStructures();}if(sectorBlend){window.gpuModels?.flush(ctx);if(!navigationOpaque)drawNearField();drawSectorBlend();}if(state==='title'){drawNavigationChart();drawShip(1090,560+Math.sin(world*.004)*8,1.3,true);for(let i=0;i<3;i++){ctx.strokeStyle='#88f8dd';ctx.globalAlpha=.2-i*.05;ctx.beginPath();ctx.moveTo(630-i*65,317+i*5);ctx.lineTo(790-i*20,317+i*5);ctx.stroke()}ctx.globalAlpha=1}else{if(!sectorBlend)drawWaterWakes();for(const e of enemies)enemyShape(e);drawBoss();window.gpuModels?.flush(ctx);if(boss)drawEncounterDefenses(boss);for(const d of drops)drawPickup(d);for(const s of shots)drawProjectile(s);for(const b of hostile)drawHostile(b);drawHazards();drawAcidClouds();drawSectorRule();drawEntryWarnings();noGlow();if(state!=='gameover'){ctx.save();ctx.globalAlpha*=ship.inv>0?.74+Math.sin(ship.inv*28)*.26:1;drawShip(ship.x,ship.y);ctx.restore();}drawFrontShield();drawWeaponOrb();for(let i=0;i<companion;i++)drawDrone(i);if(ship.shield>0){ctx.strokeStyle='#99eaff';ctx.lineWidth=2;glow('#69caff',12);ctx.beginPath();ctx.arc(ship.x,ship.y,48+Math.sin(world*.04)*3,0,TAU);ctx.stroke();noGlow()}}
...
function testLevelHotkey(e){
 ...
 const k=e.key.toLowerCase(),direct=/^[1-9]$/.test(k),travel=k==='n';
 if(!direct&&k!=='['&&k!==']'&&!travel)return false;
 e.preventDefault();if(e.repeat)return true;
 const target=direct?Number(k)-1:level+(k==='['?-1:1);
 if(target<0||target>=sectors.length){announce('NO '+(target<0?'PREVIOUS':'NEXT')+' PLANET');return true;}
 if(state!=='playing'&&state!=='paused'){start();if(travel){level=target-1;resetSector();}}
 flightRun=null;keys.clear();pointer=null;lastTouchTap=null;touchContacts.clear();pinchGesture=null;
...
function updateExpeditionIntro(){const system=contentReleases[0].systems[0],planets=system.destinations.filter(d=>d.kind==='planet'),stages=system.destinations.flatMap(d=>d.stages);$('#campaignCaption').textContent=`${system.star.name} · ${planets.length} PLANETS · ${stages.length} BOSSES · NEW GALAXY AFTER VICTORY`;const copy=$('.introcopy');if(copy)copy.textContent=`Something down there is awake.\n${planets.length} worlds orbit an unquiet sun.\nConquer each descent. Secure the ${system.name} system.`;const map=$('#expeditionMap');if(map)map.setAttribute('aria-label',`${expeditionGalaxy(expedition.locations[stages[0]]).name}, ${system.star.name} at the center, and ${planets.length} planets: ${planets.map(p=>p.name).join(', ')}`);}
